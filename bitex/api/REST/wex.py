...
class WEXREST(RESTAPI):
    ...
    def __init__(self, key=None, secret=None, version=None, config=None,
        ...
        addr = 'https://www.wex.nz/api' if not addr else addr
        super(WEXREST, self).__init__(addr=addr, version=version,
    ...
    def sign_request_kwargs(self, endpoint, **kwargs):
        ...
        WEX requires the parameters in the signature string and url to
        be appended in alphabetical order. This means we cannot rely on urllib's
        encode() method and need to do this ourselves.
        """
        req_kwargs = super(WEXREST, self).sign_request_kwargs(endpoint,
                                                                 **kwargs)
