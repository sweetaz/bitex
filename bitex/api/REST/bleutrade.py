...
class BleutradeREST(RESTAPI):
    """Bleutrade REST API class."""

    def __init__(self, key=None, secret=None, version=None, config=None,
                 addr=None, timeout=5):
        """Initialize the class instance."""
        version = 'v1' if not version else version
        addr = 'https://www.Bleutrade.com/api' if not addr else addr
        super(BleutradeREST, self).__init__(addr=addr, version=version,
    ...
    def sign_request_kwargs(self, endpoint, **kwargs):
        ...
        Bleutrade requires the parameters in the signature string and url to
        be appended in alphabetical order. This means we cannot rely on urllib's
        encode() method and need to do this ourselves.
        """
        req_kwargs = super(BleutradeREST, self).sign_request_kwargs(endpoint,
                                                                 **kwargs)
