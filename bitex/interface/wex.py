...
# Import Homebrew
from bitex.api.REST.wex import WEXREST
...
class WEX(RESTInterface):
    """WEX Interface class."""

    def __init__(self, **api_kwargs):
        """Initialize Interface class instance."""
        super(WEX, self).__init__('WEX', WEXREST(**api_kwargs))

    # pylint: disable=arguments-differ
    def request(self, endpoint, authenticate=False, **req_kwargs):
        """Generate a request to the API."""
        if authenticate:
            return super(WEX, self).request('POST', endpoint, authenticate, **req_kwargs)
        return super(WEX, self).request('GET', endpoint, authenticate, **req_kwargs)
